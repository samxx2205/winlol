...
local InVoid = false
local AutoKillStarted = false
local InVoidTimeValue = 0.04
local OnGroundTimeValue = 0.01
local lightingModifications = false
local Lighting = game:GetService("Lighting")
local lightingSettings = {
    DefaultAmbient = nil,
    DefaultOutdoorAmbient = nil,
    DefaultExposureCompensation = nil,
    DefaultFogStart = nil,
    DefaultFogEnd = nil,
    DefaultFogColor = nil,
...
-- Función para AutoKill (CON DESYNC SPAM AVANZADO INTEGRADO)
local function UpdateAutoKill()
    if not Toggles.EnableAutoKill or not Toggles.EnableAutoKill.Value then 
        -- SI DESACTIVAS EN EL VOID, VUELVE A LA POSICIÓN GUARDADA
        if AutoKillStarted and SavedPosition and LocalPlayer.Character then
    ...
    -- GUARDAR POSICIÓN ORIGINAL UNA SOLA VEZ AL ACTIVAR
    if not AutoKillStarted then
        SavedPosition = localHRP.CFrame
        AutoKillStarted = true
    end
    
    -- SISTEMA DE DESYNC SPAM ALTERNADO
    if InVoid then
        -- TELEPORTAR AL VOID PARA PROTECCIÓN
        SpoofedPosition = SavedPosition + Vector3.new(math.random(-80000, 80000), math.random(0, 80000), math.random(-80000, 80000))
        localHRP.CFrame = SpoofedPosition
        
        SetRigCollisionFalse(clone)
        SetRigTransparency(clone, 0.5)
        clone.HumanoidRootPart.CFrame = SpoofedPosition
    else
        -- VOLVER A POSICIÓN ORIGINAL Y HACER EL SPAM
        localHRP.CFrame = SavedPosition
        SetRigTransparency(clone, 1)
        
        local autoKillRange = Options.AutoKillRange and Options.AutoKillRange.Value or 50
        local autoKillSpeed = Options.AutoKillSpeed and Options.AutoKillSpeed.Value or 0.05
...
AutoKillBox:AddSlider('AutoKillSpeed', {
    Text = 'autokill speed',
    Default = 0.005,
    Min = 0.0001,
    Max = 1,
...
AutoKillBox:AddSlider('InVoidTime', {
    Text = 'void time (ms)',
    Default = 40,
    Min = 5,
    Max = 200,
    ...
    Callback = function(value)
        InVoidTimeValue = value / 1000
    end,
})

AutoKillBox:AddSlider('OnGroundTime', {
    Text = 'attack time (ms)',
    Default = 10,
    Min = 1,
    Max = 100,
    Rounding = 0,
    Callback = function(value)
        OnGroundTimeValue = value / 1000
    end,
})

-- ============ TAB HVH - TELEPORT TO SPAWN ============
local TeleportBox = Tabs.Hvh:AddRightGroupbox('teleport to spawn')

TeleportBox:AddButton('teleport', function()
    if LocalPlayer.Character then
        local humanoidRootPart = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if humanoidRootPart then
            humanoidRootPart.CFrame = CFrame.new(-335.9538879394531, 75, 14.832669258117676)
        end
    end
end)
...
-- ============ TAB VISUALS - WORLD ============
local WorldBox = Tabs.Visuals:AddLeftGroupbox('world')

WorldBox:AddToggle('LightingModifications', {
    Text = 'lighting modifications',
    Default = false,
    Callback = function(Value)
        lightingModifications = Value
        
        if not Value and Lighting then
            -- Restore default lighting
            if lightingSettings.DefaultAmbient then
                Lighting.Ambient = lightingSettings.DefaultAmbient
                Lighting.OutdoorAmbient = lightingSettings.DefaultOutdoorAmbient
                Lighting.ExposureCompensation = lightingSettings.DefaultExposureCompensation
                ...
                Lighting.ClockTime = lightingSettings.DefaultClockTime
            end
        end
    end,
})
...
WorldBox:AddLabel('Ambient Color'):AddColorPicker('AmbientColor', {
    ...
    Callback = function(Color)
        lightingSettings.CustomAmbient = Color
    end,
})

WorldBox:AddLabel('Outdoor Ambient'):AddColorPicker('OutdoorAmbientColor', {
    Default = Color3.fromRGB(80, 120, 200),
    Title = 'Outdoor Ambient',
    Callback = function(Color)
        lightingSettings.CustomOutdoorAmbient = Color
    end,
})
...
WorldBox:AddLabel('Fog Color'):AddColorPicker('FogColor', {
    ...
    Title = 'Fog',
    Callback = function(Color)
        lightingSettings.CustomFogColor = Color
    end,
})
...
-- ============ SISTEMA DE TIMING PARA DESYNC SPAM ============
task.spawn(function()
    while true do
        InVoid = true
        task.wait(InVoidTimeValue)
        InVoid = false
        task.wait(OnGroundTimeValue)
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    -- Aplicar modificaciones de iluminación
    if lightingModifications and Lighting then
        -- Guardar valores por defecto en la primera activación
        if not lightingSettings.DefaultAmbient then
            lightingSettings.DefaultAmbient = Lighting.Ambient
            lightingSettings.DefaultOutdoorAmbient = Lighting.OutdoorAmbient
            lightingSettings.DefaultExposureCompensation = Lighting.ExposureCompensation
            lightingSettings.DefaultFogStart = Lighting.FogStart
            lightingSettings.DefaultFogEnd = Lighting.FogEnd
            lightingSettings.DefaultFogColor = Lighting.FogColor
            lightingSettings.DefaultBrightness = Lighting.Brightness
            lightingSettings.DefaultClockTime = Lighting.ClockTime
        end
